-- Forward restore for the two non-additive replacements introduced by
-- 20260814193000_passive_window_engine.sql (audit BLK-2, KC-RELEASE-0.7.0-STAGE1-001-D01-R2).
-- Purpose: roll back the passive check-in Stage 1 deployment to the pre-passive
-- escalation path WITHOUT editing applied migrations (append-only rule).
-- APPLY MANUALLY ON ROLLBACK ONLY. This file deliberately lives outside
-- supabase/migrations/ so `supabase db push` can never auto-apply it.

-- 1) Restore the original private.notify_stage entry point.
--    The 14193000 migration renamed the original to notify_stage_before_passive_checkin
--    and installed a routing version. Drop the routing version, rename back.
DROP FUNCTION IF EXISTS private.notify_stage(uuid,uuid,text);
ALTER FUNCTION private.notify_stage_before_passive_checkin(uuid,uuid,text) RENAME TO notify_stage;

-- 2) Restore the pre-passive process_escalations body (original source:
--    20260808235500_alert_path_failure_isolation.sql, verbatim).
