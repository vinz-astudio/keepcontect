...
- `ACTION_POWER_CONNECTED` / `ACTION_POWER_DISCONNECTED` are **not** on the Android implicit-broadcast exemption list, and neither are `ACTION_USER_PRESENT` / `SCREEN_ON`. A manifest-declared receiver for any of the four is silently ignored on API 26+, so **no broadcast can carry charging to a dead process**.
- Charging is nevertheless collected in the background, as the 2026-08-16 human decision requires. It is collected by **observation instead of delivery**: the periodic worker reads the sticky battery state on every pass and compares it with the last confirmed stable state. A difference, re-confirmed after the same five-second stability window a live broadcast must satisfy, is a `power_transition` reported at detection time. This needs no permission, no manifest receiver and no Play declaration, and it reuses the worker whose execution is already proven in production by the coverage lease. It is the same rule iOS already carries: a transition that happens while the collector is not running is found at the next opportunity and reported when it was found, never backdated to a moment nobody observed.
- Detection latency in the background is therefore one worker period, not instant. While a KC process is alive the runtime receiver reports the change immediately. Both paths feed one confirmation routine, so neither can qualify on weaker evidence than the other.
- `ACTION_USER_PRESENT` / `SCREEN_ON` remain foreground-only extras via runtime registration; background use evidence comes from the UsageStats look-back.

  (Corrected 2026-08-16 against the official exemption list. Revision 10 asserted that POWER_* was exempt; the 0.7.1 build that implemented revision 10 moved charging to the manifest **and** deleted its runtime registration, so charging was collected nowhere at all and production held zero `power_transition` evidence from every Android install.)
