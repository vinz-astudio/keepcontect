    ...
    <main className="onboarding-flow">
      ...
      {step === 'value' && (
        <section className="onboarding-flow__hero">
          ...
          <PrototypeCard className="onboarding-flow__value-card">
            ...
            <PrototypeRow icon="notifications_active" title={isZh ? '异常时才提醒' : 'Alerts only when needed'} subtitle={isZh ? '平常不会要求你反复打卡。' : 'Normal days stay quiet.'} />
            <PrototypeRow icon="groups" title={isZh ? '由你决定谁能关照你' : 'You choose who can support you'} subtitle={isZh ? '关系与责任之后都可以调整。' : 'Relationships and responsibilities can be changed later.'} />
          </PrototypeCard>
      ...
      {step === 'phone-setup' && (
        <section className="onboarding-flow__content">
          ...
          <h1>{isZh ? '让 Keep Contact 在需要时找到你' : 'Help Keep Contact notice that you’re okay'}</h1>
          <p>{isZh ? '只处理这台手机实际支持的权限。你可以随时在 Me 页回来修复。' : 'Only settings supported by this phone are shown. You can repair them later from Me.'}</p>
          <PrototypeCard>
