-- ADR-0035 step 2: record the candidate threshold beside the live one, and
-- replay the ping history under both.
--
-- Why a replay and not just two numbers
--   A threshold is not a decision. The decision is the moment an alert opens.
--   Step 1 produced candidate values that move in both directions against the
--   live threshold -- dense accounts tighten by hours, sparse accounts loosen
--   by hours -- and no comparison of two numbers can tell anyone whether a
--   tightening catches a real emergency earlier or just fires on a quiet
--   afternoon. So this replays every gap in the window under each rule and
--   counts the alerts that would actually have opened, keeping each divergence
--   inspectable.
--
-- Record only
--   private.silence_threshold is untouched and still reads
--   public.alert_gap_profiles. Nothing here is read by any alert, notification
--   or escalation path. The scheduled job added at the bottom writes to shadow
--   tables only.
--
-- Two distinctions this file depends on, easy to conflate:
--
--   1. Sleep is subtracted when LEARNING, and suppresses when FIRING.
--      The live detector compares raw elapsed time against the threshold
--      (`now() - last_ping > threshold`) and separately refuses to fire while
--      private.sleep_relaxed is true. So this replay uses raw gap length, not
--      the sleep-adjusted length that step 1 learns from. Subtracting sleep
--      twice would invent alerts that could never fire.
--
--   2. The floor is the open question, so both answers are recorded.
--      The live rule is greatest(90 + buffer, profile + buffer): a neutral
--      floor of 90 minutes that no personalisation can go below. Whether to
--      keep it is the human's decision, and it should be answered with alert
--      counts rather than with another migration, so every row carries the
--      floored and the unfloored candidate and the alert counts for both.

-- 1) The comparison record.
CREATE TABLE public.account_threshold_shadow (
  ...
  -- EPISODES, NOT ALERT ROWS. Each count below is the number of distinct
  -- silences that would cross the rule's threshold: one per silence, however
  -- long it runs. The alerts table holds far more rows than that, because a
  -- single unbroken silence re-alerts roughly every 30 minutes after each
  -- human resolution (Karma Cheki: 101 alert rows over the same 30 days these
  -- columns score as 13 episodes). Comparing these numbers against the alerts
  -- table is apples to oranges; comparing them against each other is the point.
  gaps_evaluated integer NOT NULL CHECK (gaps_evaluated >= 0),
  episodes_live integer NOT NULL CHECK (episodes_live >= 0),
  episodes_candidate_floored integer NOT NULL CHECK (episodes_candidate_floored >= 0),
  episodes_candidate_unfloored integer NOT NULL CHECK (episodes_candidate_unfloored >= 0),
  episodes_candidate_only integer NOT NULL CHECK (episodes_candidate_only >= 0),
  episodes_live_only integer NOT NULL CHECK (episodes_live_only >= 0),

  -- Kept so a divergence can be looked at rather than trusted.
  ...
  CHECK (episodes_candidate_floored <= gaps_evaluated),
  CHECK (episodes_live <= gaps_evaluated)
);
...
BEGIN
  ...
  WITH subjects AS (
    -- One row per account that step 1 already profiled with these parameters.
    SELECT
      profile.user_id,
    ...
    SELECT
      subjects.*,
      -- Exactly the ADR-0022 assembly: additive buffer over a neutral floor.
    ...
    -- private.sleep_relaxed is true inside the sleep window and for the grace
    -- period after wake. Materialised once per account per night rather than
    -- called per candidate instant.
    SELECT
      s.user_id,
    ...
    SELECT
      ...
      gaps.ends_at,
      gaps.raw_minutes,
      -- A rule fires when the threshold elapses at an instant the detector is
      -- allowed to act on, and the account is still silent at that instant.
      -- A suppressed instant is pushed to the end of that suppression window;
      -- windows are disjoint, so one push is always enough.
...
-- 3) One daily pass over the k values step 3 will choose between.
...
-- Deliberately offset from the 02:17 adaptive-shadow maintenance job so the two
-- never contend. This job writes to account_gap_profiles and
-- account_threshold_shadow only.
DO $schedule$
BEGIN
