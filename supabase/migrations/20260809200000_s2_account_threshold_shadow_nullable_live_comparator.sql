-- ADR-0035 keeps this table as a record-only counterfactual audit. ADR-0037
-- intentionally returns no live threshold until an account has usable evidence.
-- A missing comparator is unknown, not zero, and must not erase the candidate row.

alter table public.account_threshold_shadow
  alter column live_threshold_minutes drop not null,
  alter column episodes_live drop not null,
  alter column episodes_candidate_only drop not null,
  alter column episodes_live_only drop not null;
...
BEGIN
  ...
  WITH subjects AS (
    ...
    SELECT
      ...
      gaps.ends_at,
      gaps.raw_minutes,
      thresholds.live_threshold_minutes IS NOT NULL AS has_live_comparator,
    ...
    SELECT
      ...
      CASE WHEN bool_and(fired.has_live_comparator)
        THEN count(*) FILTER (WHERE fired.fires_live)::integer
      END AS episodes_live,
  ...
  SELECT
    ...
    CASE WHEN thresholds.live_threshold_minutes IS NULL
      THEN NULL ELSE coalesce(tallied.episodes_live, 0) END,
    coalesce(tallied.episodes_candidate_floored, 0),
    coalesce(tallied.episodes_candidate_unfloored, 0),
    CASE WHEN thresholds.live_threshold_minutes IS NULL
      THEN NULL ELSE coalesce(tallied.episodes_candidate_only, 0) END,
    CASE WHEN thresholds.live_threshold_minutes IS NULL
      THEN NULL ELSE coalesce(tallied.episodes_live_only, 0) END,
...
ALTER FUNCTION private.record_account_threshold_shadow(
  date, integer, integer, numeric, integer, integer, integer, integer, integer
) OWNER TO postgres;

REVOKE ALL ON FUNCTION private.record_account_threshold_shadow(
  date, integer, integer, numeric, integer, integer, integer, integer, integer
) FROM PUBLIC;
