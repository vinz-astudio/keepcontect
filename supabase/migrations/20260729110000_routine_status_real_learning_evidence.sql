...
--
-- Before this migration the client had no server-side notion of learning progress,
-- so RoutineInsights derived it from (now - installedAt) and, after 14 calendar days,
-- displayed 「已建立作息基线」 even though no baseline existed and nothing influenced
-- alerting. This adds evidence counters sourced from the adaptive pipeline itself.
--
-- Boundaries: read-only additive change. This does NOT touch private.silence_threshold,
-- does NOT give learned output any live alert authority (ADR-0022 remains sole live
-- authority), and does NOT alter the existing response keys. The legacy
-- model_confidence/model_explanation/model_version keys are retained for contract
-- compatibility but are now explicitly superseded by the evidence_* keys; they were
-- never read by any client.
...
BEGIN
  ...
  -- Learning is only "active" when an enabled shadow version actually exists.
  SELECT c.version_id
    INTO _version_id
  ...
  IF _version_id IS NOT NULL THEN
    ...
    -- personal_global is the user's own cross-context baseline; the per-context
    -- rows are narrower and would understate overall progress.
  ...
  RETURN jsonb_build_object(
    ...
    -- Superseded by evidence_* below; retained for contract compatibility only.
    'model_confidence', _model_confidence,
    'model_explanation', _model_explanation,
    'model_version', _model_version,
    -- ADR-0029 P5 real evidence.
