...
SELECT is((SELECT array_to_string(proconfig, ',') FROM pg_proc WHERE oid='private.qualified_behavior_sessions(uuid,timestamptz,timestamptz,uuid)'::regprocedure), 'search_path=""', 'sessions pin empty search path');
SELECT is((SELECT array_to_string(proconfig, ',') FROM pg_proc WHERE oid='private.rebuild_alert_gap_profiles(uuid,date)'::regprocedure), 'search_path=""', 'rebuild pins empty search path');
