-- Sensitivity tool buffers changed from +15m/+30m/+90m to +0m/+45m/+90m.
-- "Sensitive" now maps to the neutral model value as-is (no buffer); the front
-- end labels each option with its buffer (+0m / +45m / +90m) so users can tell
-- them apart. Floors and the 12h cap are unchanged.
