...
-- 2) The rebuild.
--    The sleep rule lives here once, set-based. An earlier draft called a
--    per-gap helper function; at production volume that meant one settings
--    lookup and one timezone-catalogue scan per gap and the rebuild did not
--    finish. It is also the shape this project keeps getting bitten by: one
--    rule with two implementations eventually disagree.
