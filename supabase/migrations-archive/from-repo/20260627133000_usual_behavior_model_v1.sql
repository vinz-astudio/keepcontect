-- Personal Usual Behavior Model v1.
-- Add model metadata while keeping hourly_thresholds/weekend_multiplier compatible.
-- Sensitivity is a user-facing tool applied after the neutral model threshold.

alter table public.user_activity_profiles
  add column if not exists model_version text not null default 'hourly_threshold_v1',
  add column if not exists model_confidence double precision,
  add column if not exists hourly_confidence double precision[],
  add column if not exists gap_stats jsonb,
...
begin
  ...
  if _threshold is not null then
    ...
    -- Tool semantics:
    -- high/sensitive ~= model threshold + 30m
    -- balanced waits longer
    -- low/relaxed waits longest
    if _s = 'high' then
      _threshold := _threshold + 0.5;
      _floor := 1.0;
    elsif _s = 'low' then
      _threshold := _threshold * 2.2 + 1.0;
      _floor := 6.0;
    else
      _threshold := _threshold * 1.35 + 0.5;
      _floor := 2.0;
    end if;
