...
--
-- Start the adaptive shadow from retained, sessionized ingest_version=1
-- history and extend the same rolling profiles with later canonical-v2
-- evidence. The learned path remains shadow-only: this migration does not
-- change private.silence_threshold or write any live alert/notification row.
  ...
  BEGIN
    ...
    IF NOT FOUND
       ...
       OR _version.config_sha256 <> encode(
         extensions.digest(_version.config::text, 'sha256'),
         'hex'
       ) THEN
  ...
  EXCEPTION
    WHEN OTHERS THEN
      _failure_code := CASE
        ...
        WHEN SQLERRM LIKE '%shadow_history_policy_validation_failed%'
          THEN 'shadow_malformed_result'
        WHEN SQLERRM LIKE '%shadow_version_validation_failed%'
          THEN 'shadow_malformed_result'
...
BEGIN
  ...
  SELECT runtime.version_id
    INTO _old_version_id
  FROM private.adaptive_alert_shadow_runtime_config AS runtime
  WHERE runtime.singleton;
  ...
  PERFORM private.rebuild_alert_gap_profiles(
    _version_id,
    (_activation_minute AT TIME ZONE 'UTC')::date
  );
  ...
  _cycle_result := private.run_adaptive_alert_shadow_cycle(
    _version_id,
    _activation_minute
  );
